...
- **Every GitHub Release MUST ship with a full docs sync — no exceptions (user mandate, v0.15.2).** Before publishing a release, update ALL user-facing documentation to match exactly what the release contains, in both languages:
  - `README.md` + `README.zh-CN.md` — feature tables / usage sections for every user-visible feature added since the last release;
  - F1 help (`src/lib/i18n.ts` `helpSections`) — new syntax, menus, and settings paragraphs in **both en and zh**;
  - `docs/BACKLOG.md` — move finished items into 已实现 with version tags, record pitfalls in 技术备忘.
  A release whose docs are stale is an incomplete release: finish the docs BEFORE creating the release on GitHub.
