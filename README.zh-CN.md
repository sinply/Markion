...
一款快速、**本地优先**的 **Windows** Markdown 知识库，按语雀的方式打造：单一无缝视图——文档始终以渲染后的样子呈现，点上去直接写，无需"编辑/预览"切换。文件夹可作文档容器或自动数据表，标签来自笔记属性；所有内容都是磁盘上的普通 `.md` 文件。无私有数据库、无锁定，可轻松备份并配合你已有的任意同步方式使用。macOS 与 Linux 版本规划中（应用基于跨平台的 Tauri 2 构建）。
...
| | |
|---|---|
| 🪄 **单一无缝视图** | 语雀式——文档始终渲染：标题、表格、代码卡片所见即所得，光标所在处直接显示源码供编辑。没有"编辑 ⇄ 预览"切换。 |
| 🖱️ **多光标 / 列选择** | Alt+点击添加光标，Shift+Alt+拖拽进行列选择。 |
| 🎯 **`[[note#heading]]` 锚点跳转** | 带 `#heading` 锚点的 wiki 链接正确解析（ATX 与 Setext 标题均支持），打开后自动滚动定位到对应标题。 |
| 🔗 **Wikilink 自动补全** | 输入 `[[` 即按 前缀 → 词边界 → 子串 三级排序推荐笔记名，精确名不存在时始终附带「新建笔记」入口。 |
| 🧘 **专注模式** | 光标所在段落保持清晰并带主题色侧条，视区内其余内容淡出，两侧面板自动收起，写作零干扰。 |
| 📑 **标题折叠** | 折叠槽 + 快捷键收起/展开章节。 |
...
| | |
|---|---|
| 🌲 **多级文档树** | 语雀式嵌套文件夹，拖拽排序/跨文件夹移动，右键菜单（新建/重命名/删除→回收站），隐藏 dotfiles（设置中可开关）。 |
| 🏢 **文档容器** | 含 `index.md` 的文件夹点击即作为一页打开，子文档列在编辑器下方——可切换为表格视图。 |
| 🗃️ **文件夹即数据库** | 任意文件夹可将笔记渲染成数据表：列类型按 frontmatter 键自动推断（数字/日期/标签/文本），可排序筛选，双击单元格直接写回笔记 YAML；也可右键"查看为表格"。 |
| 🏷️ **属性即标签** | 标签是笔记元数据：来自 YAML 头部的 `tags:` 属性，标签面板据此筛选——正文里的 `#碎片` 不会污染列表。 |
| 🧭 **大纲 / 反链 / 关系图谱** | 可切换右侧面板：实时大纲（可拖拽标题排序）、`[[wikilink]]` 反链、可缩放的关系图谱。 |
| 🏢 **多知识库** | 文件菜单切换最近打开的知识库；可固定默认知识库自动打开。 |
...
| 📝 **模板与每日笔记** | 可配置模板文件夹；"打开今日笔记"入口为可选项（设置中开启）。 |
| 🏷️ **属性面板** | YAML frontmatter 可视化编辑（标题/标签/日期/自定义键值）。 |
| 📋 **目录 TOC 插入** | 一键插入按标题层级生成的、可点击跳转的目录（ATX 与 Setext 标题）。 |
| 🔧 **快捷键自定义** | 偏好设置中任意重绑快捷键（输入 `-` 可解绑），冲突的重复绑定会被拒绝；按知识库存储、即时生效。 |
