...
| 🌲 **多级文档树** | 语雀式嵌套文件夹，支持折叠/展开、双击文件夹、拖拽排序与跨文件夹移动，可隐藏 dotfiles。 |
| 🧭 **大纲 / 反链 / 关系图谱** | 可切换的右侧面板：实时大纲、`[[wikilink]]` 反链、可缩放/平移的笔记关系图谱。 |
| ✅ **GFM + 扩展** | 表格、任务列表、删除线、语法高亮、Mermaid 图表、KaTeX 公式（`$$..$$` 与 `$..$`）、YAML frontmatter。 |
...
| 📁 **本地优先** | 直接读写磁盘上的 `.md` 文件，无私有数据库、无锁定——启动时自动打开默认仓库。 |
...
**已完成**
- [x] 实时预览（标题/粗体/斜体/行内代码/链接/表格/任务列表/代码块/引用/图片/数学公式）
- [x] 多级文件树 + 拖拽 + 双击展开 + 隐藏 dotfiles
- [x] 大纲 / 反链 / 关系图谱面板（可切换）+ 图谱缩放与平移
- [x] 图片粘贴/拖拽 UI 接入
- [x] 外部文件变更监听（实时树刷新 + 重载）
- [x] 设置持久化（`.markion/config.json`）+ 默认仓库自动打开
- [x] Mermaid 图表 + KaTeX 数学公式（块级 `$$...$$` + 行内 `$...$`）
- [x] 反链面板（`[[wikilink]]` 反向查找）
- [x] 菜单栏：文件 · 编辑 · 格式 · 视图 · 帮助（中英双语）
- [x] 11 种主题 + 字体选择 + 字数统计
- [x] 应用内置文档（F1）
